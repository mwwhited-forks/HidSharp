...
{
    abstract class SysUsbTmcStream : UsbTmcStream
    {
        protected SysUsbTmcStream(UsbTmcDevice device)
            : base(device)
        {
        ...
        internal void CommonDisconnected(Queue<byte[]> readQueue)
        {
            lock (readQueue)
            {
                if (readQueue.Count == 0 || null != readQueue.Peek())
                { readQueue.Enqueue(null); Monitor.PulseAll(readQueue); }
            }
        }
            ...
            try
            {
                lock (queue)
                {
                    while (true)
                    {
                        ...
                        {
                            if (null == queue.Peek())
                            { throw new IOException("I/O disconnected."); } // Disconnected.
