...
namespace HidSharp.Platform.Windows
{
    sealed partial class WinUsbTmcDevice : UsbTmcDevice
    {
        ...
        bool TryGetDeviceString(IntPtr handle, Func<IntPtr, char[], int, bool> callback, out string s)
        {
            ...
            s = NativeMethods.NTString(buffer);
            return true;
        }
        ...
        public override byte[] GetRawReportDescriptor()
        {
            ...
            if (descriptor == null)
            { throw new NotSupportedException("Unable to reconstruct the report descriptor."); } // TODO: Extend the reconstruction functionality over time...
        ...
        bool TryGetDeviceUsbRoot(out uint devInst)
        {
            if (0 == NativeMethods.CM_Locate_DevNode(out devInst, _id))
            {
                ...
                while (true)
                {
                    ...
                    if (Regex.IsMatch(parentDeviceID, @"^USB\\VID_[0-9A-F]{4}&PID_[0-9A-F]{4}\\"))
                    { return true; }
                }
            }
